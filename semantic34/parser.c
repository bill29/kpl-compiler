...
void compileFuncDecl(void) {
  ...
  strcpy(Functionname,currentToken->string);
  ...
  checkFunctionreturn = 0;
  compileBlock();
  if(checkFunctionreturn == 0){
      error(ERR_THIEU_GIA_TRI_TRA_VE, lookAhead->lineNo, lookAhead->colNo);
  }
  eat(SB_SEMICOLON);
 // checkFuntionReturn(funcObj->name);
...
void compileStatement(void) {
  switch (lookAhead->tokenType) {
  case TK_IDENT:
    // printf("%s - %s\n",lookAhead->string,Functionname);
    if(strcmp(lookAhead->string,Functionname) == 0 ){
      //printf("%s ++ \n",lookAhead->string);
      checkFunctionreturn ++ ;
    }
    compileAssignSt();
...
void compileForSt(void) {
  ...
  if (checkDeclaredVariable(currentToken->string) == NULL)
    ...
    if(checkDeclaredVariable(currentToken->string)->varAttrs->type->typeClass == TP_FLOAT){
      error(ERR_SAI_KIEU, currentToken->lineNo, currentToken->colNo);
    }
  }
  eat(SB_ASSIGN);
  ...
  if(lookAhead->tokenType == TK_IDENT){
    if (checkDeclaredVariable(lookAhead->string) == NULL)
      error(ERR_SAI_KIEU, currentToken->lineNo, currentToken->colNo);
    else
    {
      error(ERR_SAI_KIEU, currentToken->lineNo, currentToken->colNo);
